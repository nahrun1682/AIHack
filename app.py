...
from core.llm import is_api_key_configured
from core.upgrades import get_rarity_color
from ui.components import render_sidebar, execute_conversation
...
def render_game_screen():
    ...
    with col_left:
        ...
        # Initialize session state if not set (first load or page refresh)
        if "user_prompt_input" not in st.session_state:
            st.session_state["user_prompt_input"] = engine.player_prompt

        prompt = st.text_area(
            "システムプロンプト",
            key="user_prompt_input", # Use key for IME stability
        ...
        # Update engine whenever prompt changes
        if prompt != engine.player_prompt:
